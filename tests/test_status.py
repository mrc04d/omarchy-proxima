...
class StatusContract(unittest.TestCase):
    def test_three_of_four_logged_in(self):
        threading.Thread(target=make_serve({"claude": True, "chatgpt": True, "gemini": False, "perplexity": True}), daemon=True).start()
    ...
    def test_all_four_logged_in(self):
        threading.Thread(target=make_serve({"claude": True, "chatgpt": True, "gemini": True, "perplexity": True}), daemon=True).start()
        env = dict(os.environ, PROXIMA_TEST_PORT=str(PORT))
        out = subprocess.run(
            [sys.executable, HELPER, "status"],
            capture_output=True,
        ...
        data = json.loads(out.stdout.strip().splitlines()[-1])
        self.assertTrue(data["alive"], data)
        self.assertEqual(data["count"], 4, data)
        self.assertTrue(data["allLoggedIn"], data)
        self.assertTrue(data["providers"]["gemini"], data)
        self.assertTrue(data["providers"]["claude"], data)
